...
void play_with_texture_4(SDL_Texture* my_texture,
       ...
       int nb_images = 26;                     // Il y a 8 vignette dans la ligne de l'image qui nous intéresse
       ...
       state.x = offset_x;                // On s'intéresse à la 4ème ligne, le bonhomme qui court
...
void play_with_texture_1_1(SDL_Texture *my_texture, SDL_Window *window, SDL_Renderer *renderer) 
{
 ...
 void play_with_texture_5(SDL_Texture *bg_texture,
  ...
  int nb_images = 26;                         //  Il y a 8 vignette dans la ligne qui nous intéresse
  ...
  SDL_Rect state[27];                         // Tableau qui stocke les vignettes dans le bon ordre pour l'animation
  ...
  int i = 0;                                   
    ...
    for (int x = 0; x < source.w; x += offset_x) {
      ...
      ++i;
                        ...
                        // state[14 à 16] la même image, le monstre ne bouge pas   
  ...
  destination.w = offset_x * zoom;            // Largeur du sprite à l'écran
  destination.h = offset_y * zoom;            // Hauteur du sprite à l'écran
  destination.x =  1365; // 500;//window_dimensions.w * 0.75; // Position en x pour l'affichage du sprite 1400;
  destination.y = 55;//window_dimensions.h * 0.7;       //110 Position en y pour l'affichage du sprite
...
void play_with_texture_mon_animation(SDL_Texture* my_texture, SDL_Window* window, SDL_Renderer* renderer) 
{
  SDL_Rect 
        source = {0},                             // Rectangle définissant la zone de la texture à récupérer
        window_dimensions = {0},                  // Rectangle définissant la fenêtre, on n'utilisera que largeur et hauteur
  ...
  int nb_it = 200;                                // Nombre d'images de l'animation
                  // On applique le zoom sur la hauteur
  destination.x = (window_dimensions.w- destination.w) / 8;
  ...
  for (int i = 0; i < nb_it; ++i) 
  {
    ...
    destination.h = source.h * zoom;
     if (i%50<25)
    {
        destination.y = (window_dimensions.h - destination.h) / 2 + i%50 ;
    }                         
        
    else  destination.y = (window_dimensions.h - destination.h) / 2 - i%50;
